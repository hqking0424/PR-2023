...
# constant
IMG_SCALE = 1./255
IMG_MEAN = [0.3714,0.3609, 0.2386]
IMG_STD = [0.2705 ,0.2567 ,0.2161]
SCALES = [0.7, 1, 1.3]
SHORTER_SIDE = 224

# system-related parameters 系统相关函数
DATA_DIR = './data/sorghum_head_counting_dataset'#路径
DATASET = 'shc'#数据集
EXP = 'tasselnetv2plus'
DATA_LIST = './data/sorghum_head_counting_dataset/dataset1_train.txt'
DATA_VAL_LIST = './data/sorghum_head_counting_dataset/dataset1_test.txt'
...
# training-related parameters 训练相关参数
...
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")#有GPU则使用GPU
...
def save_checkpoint(state, snapshot_dir, filename='model_ckpt.pth.tar'):#保存权重
    torch.save(state, '{}/{}'.format(snapshot_dir, filename))

def plot_learning_curves(net, dir_to_save):#绘制学习曲线
...
def train(net, train_loader, criterion, optimizer, epoch, args):#训练
    # switch to 'train' mode
    net.train()#训练网络定义

    # uncomment the following line if the training images don't have the same size
    cudnn.benchmark = True#开启benchmark
    ...
    in_sz = args.input_size#输入尺寸
    os = args.output_stride#输出
    target_filter = torch.FloatTensor(1, 1, in_sz, in_sz).fill_(1)#数据转换
    for i, sample in enumerate(train_loader):
        #torch.cuda.synchronize()
        start = time()

        inputs, targets = sample['image'], sample['target']#获取数据中的输入图片和目标
...
def validate(net, valset, val_loader, criterion, epoch, args):#测试
    # switch to 'eval' mode
    net.eval()
    cudnn.benchmark = False
    early_stopping = EarlyStopping(args.snapshot_dir)#为hqking新加部分，早停
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(val_loader):
           ...
           # torch.synchronize()
            ...
            start = time()
            #early_stopping(net.measure['mae'][-1], net)
...
def main():
    ...
    # instantiate dataset
    dataset = dataset_list[args.dataset]#加载数据集
    
    args.snapshot_dir = os.path.join(args.snapshot_dir, args.dataset.lower(), args.exp)#snapshot文件路径，若没有则创建
    if not os.path.exists(args.snapshot_dir):
        os.makedirs(args.snapshot_dir)

    args.result_dir = os.path.join(args.result_dir, args.dataset.lower(), args.exp)#结果保存文件，没有则创建
    ...
    # instantiate network
    net = CountingModels(#加载网络
        arc=args.model,
        input_size=args.input_size,
        output_stride=args.output_stride
    )

    net = nn.DataParallel(net)#多GPU？对于我们貌似没啥用
    #net.cuda()
    ...
    if args.optimizer == 'sgd':#sgd方法
        ...
        optimizer = torch.optim.Adam(#adam方法
            [
                {'params': learning_params},
                {'params': pretrained_params, 'lr': args.learning_rate / args.mult},
            ],
    ...
    if args.restore_from is not None:#加载checkpoint
    ...
    # define transform
    transform_train = [#数据增强部分，可增添其他的试试
        RandomCrop(args.crop_size),#裁剪
        RandomFlip(),#翻转
        Normalize(#归一化
            args.image_scale, 
            args.image_mean, #平均值
