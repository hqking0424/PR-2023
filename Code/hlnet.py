    ...
    def __init__(self, arc='tasselnetv2plus'):
        ...
        if arc == 'tasselnetv2':
            ...
            self.encoder = nn.Sequential(
                nn.Conv2d(3, 16, 3, padding=1, bias=False),
                nn.BatchNorm2d(16),
                nn.ReLU(inplace=True), #Conv1
                nn.MaxPool2d((2, 2), stride=2), #Maxpool1

                nn.Conv2d(16, 32, 3, padding=1, bias=False),
                nn.BatchNorm2d(32),
                nn.ReLU(inplace=True), #Conv2
                nn.MaxPool2d((2, 2), stride=2), #Maxpool2

                nn.Conv2d(32, 64, 3, padding=1, bias=False),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True), #Conv3
                nn.MaxPool2d((2, 2), stride=2), #Maxpool3
...
class Counter(nn.Module):
    def __init__(self, arc='tasselnetv2plus', input_size=64, output_stride=8):
        ...
        if arc == 'tasselnetv2':
            ...
            self.counter = nn.Sequential(
                nn.AvgPool2d((k, k), stride=avg_pool_stride), #全平均池化
                nn.Conv2d(128, 128, 1, bias=False),
                nn.BatchNorm2d(128),
                nn.ReLU(inplace=True), #Conv6
                nn.Conv2d(128, 1, 1) #归一化
            )
        else:
            raise NotImplementedError
...
if __name__ == "__main__":
    ...
    insz, os = 64, 8
    imH, imW = 1080, 1920
    net = CountingModels(arc='tasselnetv2plus', input_size=insz, output_stride=os)
    with torch.no_grad():
        net.eval()
